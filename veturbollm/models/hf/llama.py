...
def remap_state_dict_meta_llama(state_dict, config):
    ...
    def key_mapping_mlp(key):
        return re.sub(r"^transformer.layers.(\d+).feed_forward.w2.", r"transformer.layers.\1.mlp.fc2.", key)
    ...
    def key_mapping_attn(key):
        return re.sub(r"^transformer.layers.(\d+).attention.wo.", r"transformer.layers.\1.mixer.out_proj.", key)
...
def remap_state_dict_hf_llama(state_dict, config):
    ...
    # Word embedding
    def key_mapping_emb(key):
        return re.sub(r"^transformer.model.embed_tokens.", "transformer.embeddings.word_embeddings.", key)
    ...
    def key_mapping_mlp(key):
        return re.sub(r"^transformer.model.layers.(\d+).mlp.down_proj.", r"transformer.layers.\1.mlp.fc2.", key)
