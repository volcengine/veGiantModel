...
def build_num_microbatches_calculator(args):
    ...
    if args.training.rampup_batch_size is None:
        ...
        assert len(args.training.rampup_batch_size) == 3, (
            "expected the following "
            "format: --rampup-batch-size <start batch size> "
            "<batch size incerement> <ramp-up samples>"
        )
        start_batch_size = int(args.training.rampup_batch_size[0])
        batch_size_increment = int(args.training.rampup_batch_size[1])
        ramup_samples = int(args.training.rampup_batch_size[2])
        if args.rank == 0:
            print(
