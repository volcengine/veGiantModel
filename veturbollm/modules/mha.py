...
class MHA(nn.Module):
        ...
        if not self.cross_attn:
            ...
            if self.dwconv:
                ...
                self.dwconv_kv = nn.Conv1d(
                    2 * embed_dim, 2 * embed_dim, kernel_size=3, padding=2, groups=2 * embed_dim
                )
    ...
    def allocate_inference_cache(self, batch_size, max_seqlen, dtype=None, fused_ft_kernel=True):
        ...
        if not fused_ft_kernel:
            ...
            k_cache = torch.empty(
                batch_size, self.num_heads, self.head_dim // packsize, max_seqlen, packsize, dtype=dtype, device=device
            )
            v_cache = torch.empty(batch_size, self.num_heads, max_seqlen, self.head_dim, dtype=dtype, device=device)
